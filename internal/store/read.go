...
func (s *Store) GetRunsByPipeline(ctx context.Context, pipeline string) ([]Run, error) {
	rows, err := s.pool.Query(ctx,
		`SELECT pipeline, run_no, start_time, end_time, status, COALESCE(git_hash,''), COALESCE(git_branch,''), COALESCE(git_repo,'')
	...
	for rows.Next() {
		var r Run
		err := rows.Scan(&r.Pipeline, &r.RunNo, &r.StartTime, &r.EndTime, &r.Status, &r.GitHash, &r.GitBranch, &r.GitRepo)
		if err != nil {
			return nil, err
		}
		runs = append(runs, r)
...
func (s *Store) GetRun(ctx context.Context, pipeline string, runNo int) (*Run, error) {
	...
	err := s.pool.QueryRow(ctx,
		...
		pipeline, runNo,
	).Scan(&r.Pipeline, &r.RunNo, &r.StartTime, &r.EndTime, &r.Status, &r.GitHash, &r.GitBranch, &r.GitRepo)
	if err != nil {
